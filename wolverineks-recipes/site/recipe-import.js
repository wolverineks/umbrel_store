...
exports.RECIPE_JSON_SCHEMA = void 0;
exports.fetchRecipePage = fetchRecipePage;
exports.formatRecipeWithGrok = formatRecipeWithGrok;
const MAX_TEXT_LENGTH = 12000;
const FETCH_TIMEOUT_MS = 20000;
exports.RECIPE_JSON_SCHEMA = {
    type: "object",
    properties: {
...
function metaContent(html, attr, value) {
    ...
    const pattern = new RegExp(`<meta[^>]+${attr}=["']${escaped}["'][^>]+content=["']([^"']+)["']|<meta[^>]+content=["']([^"']+)["'][^>]+${attr}=["']${escaped}["']`, "i");
...
function serializeJsonLd(recipe) {
    ...
    for (const field of fields) {
        ...
        if (Array.isArray(value)) {
            ...
            const record = value;
            if (typeof record.text === "string")
                lines.push(`${field}: ${record.text}`);
        }
    }
    return lines.join("\n");
}
function extractImageUrl(html, jsonLd, pageUrl) {
    if (jsonLd?.image) {
    ...
    if (fromOg && !fromOg.startsWith("data:"))
        return fromOg;
    const twitterImage = metaContent(html, "name", "twitter:image");
    const fromTwitter = absolutizeUrl(twitterImage, pageUrl);
    if (fromTwitter && !fromTwitter.startsWith("data:"))
        return fromTwitter;
    return null;
}
...
async function formatRecipeWithGrok(raw, apiKey, model) {
    const response = await fetch("https://api.x.ai/v1/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            response_format: {
                ...
                json_schema: {
                    name: "recipe",
                    schema: exports.RECIPE_JSON_SCHEMA,
                    strict: true,
                },
            },
        }),
