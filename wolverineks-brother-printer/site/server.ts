...
const IPP_ATTRS_TEST_PATH =
  process.env.IPP_ATTRS_TEST_PATH?.trim() || "/usr/share/cups/ipptool/get-printer-attributes.test";
...
function formatMediaLabel(media: string): string {
  const trimmed = media.trim();
  return MEDIA_LABELS[trimmed] ?? trimmed.replace(/^(na_|iso_|oe_|om_)/, "").replace(/_/g, " ");
}

function formatAdfState(state: string): string {
...
function parseIppKeywordList(value: string | string[] | null | undefined): string[] {
  if (!value) return [];
  const parts = Array.isArray(value) ? value : [value];
  return [...new Set(parts.flatMap((entry) => entry.split(",").map((item) => item.trim()).filter(Boolean)))];
}

type IppStatusSummary = {
  available: boolean;
  error?: string;
...
const ippStatusCache = new Map<string, { at: number; data: IppStatusSummary }>();
const IPP_STATUS_CACHE_MS = 30_000;

async function getIppStatusSummary(host: string): Promise<IppStatusSummary> {
  const cached = ippStatusCache.get(host);
  if (cached && Date.now() - cached.at < IPP_STATUS_CACHE_MS) {
    return cached.data;
  }

  const empty: IppStatusSummary = {
    available: false,
    media_ready: null,
    media_default: null,
    media_supported: [],
  ...
  const summary: IppStatusSummary = {
    available: true,
...
function buildIppAlerts(ipp: IppStatusSummary): StatusAlert[] {
  if (!ipp.available) return [];
...
function mergeStatusAlerts(primary: StatusAlert[], extra: StatusAlert[]): StatusAlert[] {
  const merged = [...primary];
  for (const alert of extra) {
    const duplicate = merged.some(
      (existing) => existing.title === alert.title || existing.message === alert.message,
    );
    if (!duplicate) merged.push(alert);
  }
  return merged;
}
...
function parsePageYield(html: string): PageYield | null {
  const section = html.match(/<table id="possiblePrintNum"[\s\S]*?<\/table>/i)?.[0];
  if (!section) return null;
  const values = [...section.matchAll(/<th><span>([^<]*)<\/span><\/th>/gi)].map((match) =>
...
function parseStatusPageModel(html: string): string | null {
  const titleMatch = html.match(/<title>([^<]+)<\/title>/i);
  return titleMatch ? decodeHtmlEntities(titleMatch[1].trim()) : null;
}

function parseStatusPageMonitor(html: string): { message: string; level: DeviceStatusLevel } {
  const match = html.match(/<span class="moni\s+([^"]+)"[^>]*>([\s\S]*?)<\/span>/i);
  if (!match) {
    return { message: "Status unavailable", level: "unknown" };
  }
...
type EsclScanJob = {
  ...
  images_to_transfer: number;
  job_state: string | null;
  job_state_reason: string | null;
};

function parseEsclScanJobs(xml: string): EsclScanJob[] {
...
function parseEsclCapabilitiesExtended(xml: string) {
  return {
...
function parseIppToolOutput(output: string): Record<string, string | string[]> {
...
function ippAttrValue(attrs: Record<string, string | string[]>, key: string): string | null {
...
type ParsedPrinterSupply = {
  type: string;
  type_label: string;
  colorant_name: string | null;
  label: string;
  level: number | null;
...
const SUPPLY_CLASS_LABELS: Record<string, string> = {
...
function splitPrinterSupplyEntries(raw: string): string[] {
...
function parseSupplyDescriptions(value: string | string[] | null | undefined): string[] {
  if (!value) return [];
  const parts = Array.isArray(value) ? value : [value];
  return parts.flatMap((entry) => entry.split(",").map((item) => item.trim()).filter(Boolean));
}

function formatSupplyLevel(level: number | null, unit: string | null): string {
  if (level === null || Number.isNaN(level)) return "Unknown";
  if (level === -3) return "Not reported by printer";
  if (level === -2) return "Unknown";
  if (level === -1) return "Other";
  if (level === 0) return "Empty";
  if (unit === "tenthsOfMilliliters") return `${(level / 10).toFixed(1)} ml remaining`;
  if (unit === "percent") return `${level}% remaining`;
  if (unit === "pages") return `${level} pages remaining`;
  return `${level} remaining`;
}

function formatSupplyCapacity(maxCapacity: number | null, unit: string | null): string {
  if (maxCapacity === null || Number.isNaN(maxCapacity)) return "Unknown";
  if (maxCapacity <= -2) return "Not reported";
  if (maxCapacity === -1) return "Other";
  if (maxCapacity === 0) return "None";
  if (unit === "tenthsOfMilliliters") return `${(maxCapacity / 10).toFixed(1)} ml capacity`;
  if (unit === "percent") return `${maxCapacity}% scale`;
  if (unit === "pages") return `${maxCapacity} page capacity`;
  return String(maxCapacity);
}

function inferSupplyLevelHint(level: number | null): string {
  if (level === -3) return "Printer did not report a numeric level (common when ink is present).";
  if (level === 0) return "Printer reports this supply is empty.";
  if (level != null && level > 0) return "Numeric level reported by the printer.";
  return "";
}

function parsePrinterSupplyEntry(entry: string, description?: string): ParsedPrinterSupply {
...
function parsePrinterSupplies(
  ...
  return splitPrinterSupplyEntries(rawText).map((entry, index) =>
    parsePrinterSupplyEntry(entry, descriptionList[index]),
  );
}

async function probePrinterEndpoint(host: string, route: string): Promise<EndpointProbe> {
...
async function getIppDiagnostics(host: string): Promise<{
  available: boolean;
  ...
  if (!existsSync(IPP_ATTRS_TEST_PATH)) {
    return {
      available: false,
      attributes: {},
  ...
  const result = await runCommand(
    IPPTOOL_PATH,
    ["-tv", "-d", `uri=${ippUri(host)}`, IPP_ATTRS_TEST_PATH],
    20000,
  );
...
async function getLibraryDiagnostics(): Promise<{
  scan_count: number;
  total_bytes: number;
...
async function getDiagnostics(settings: Settings) {
  ...
  let printer: Awaited<ReturnType<typeof getPrinterStatus>> | null = null;
  let capabilities = null as ReturnType<typeof getScannerCapabilities> extends Promise<infer T> ? T | null : null;
  let scannerStatusXml: string | null = null;
  let capabilitiesXml: string | null = null;
  let statusHtml: string | null = null;
  let monitorHtml: string | null = null;
  let statusPageMonitor: ReturnType<typeof parseStatusPageMonitor> | null = null;
  let pageYield: PageYield | null = null;
  let statusPageModel: string | null = null;
  let esclCapabilities: ReturnType<typeof parseEsclCapabilitiesExtended> | null = null;
  let esclScanJobs: EsclScanJob[] = [];
  ...
  let ipp: Awaited<ReturnType<typeof getIppDiagnostics>> = {
    available: false,
    attributes: {},
  };
  try {
  ...
  return {
    ...
    app: {
      ...
      runtime: {
        ...
        scans_dir_exists: existsSync(SCANS_DIR),
        ipptool_path: IPPTOOL_PATH,
        ipptool_available: existsSync(IPPTOOL_PATH),
        pdftoppm_path: PDFTOPPM_PATH,
        pdftoppm_available: existsSync(PDFTOPPM_PATH),
        ipp_attrs_test_path: IPP_ATTRS_TEST_PATH,
        ipp_attrs_test_available: existsSync(IPP_ATTRS_TEST_PATH),
      },
      library,
    },
    printer: {
      ...
      ipp: {
        ...
        supplies_parsed: parsePrinterSupplies(
          ipp.attributes["printer-supply"],
          ipp.attributes["printer-supply-description"],
        ),
...
const API_RESPONSE_SCHEMAS: Record<string, unknown> = {
  "GET /api/status": {
    settings: {
...
function renderResponseSchemaPanels(names: string[]): string {
  return names
    .filter((name) => name in API_RESPONSE_SCHEMAS)
    .map(
      (name) =>
        `<details class="json-panel"><summary>${escapeHtml(name)}</summary><pre>${escapeHtml(JSON.stringify(API_RESPONSE_SCHEMAS[name], null, 2))}</pre></details>`,
    )
    .join("");
}
...
function buildExplorerPayload(settings: Settings) {
