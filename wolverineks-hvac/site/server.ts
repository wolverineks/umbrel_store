...
type ZoneView = {
  ...
  fan_setting_display: string | null;
  ...
  operation_kind: ZoneOperationKind;
  operation_stage: number;
  operation_transition: boolean;
};

type ZoneOperationKind = "idle" | "heating" | "cooling" | "fan" | "circulating" | "defrost" | "transition";

type ConditioningCategory = "idle" | "heat" | "cool" | "fan" | "prep" | "prep_heat" | "prep_cool" | string;

type CompressorStageParse = {
  stage: number;
...
const ACTIVITY_DISPLAY_ORDER = ["home", "wake", "sleep", "away", "vacation", "manual"] as const;
...
const operationHistoryByZone = new Map<string, OperationSample[]>();
      ...
      if (applyInfinityStatusMessage(cachedSystems, message)) {
        ...
        if (liveSystem) {
          for (const statusZone of liveSystem.status.zones) {
            if (statusZone.enabled !== "on") continue;
...
function parseCompressorStage(
  opstat: string | null | undefined,
  conditioning: string,
  systemMode: string,
): CompressorStageParse {
  ...
  let heatCool: "heat" | "cool" | null = null;
  if (value.includes("heat")) heatCool = "heat";
  else if (value.includes("cool")) heatCool = "cool";
  const cond = conditioning.toLowerCase();
  if (!heatCool && cond.includes("heat")) heatCool = "heat";
  else if (!heatCool && cond.includes("cool")) heatCool = "cool";
  else if (!heatCool && stage > 0) {
    const mode = systemMode.toLowerCase();
    if (mode === "heat" || mode === "emheat") heatCool = "heat";
    else if (mode === "cool") heatCool = "cool";
  }
...
function conditioningCategory(conditioning: string): ConditioningCategory {
  const value = String(conditioning || "idle").toLowerCase();
  if (value.includes("prep") || value.includes("pending")) {
    if (value.includes("heat")) return "prep_heat";
    if (value.includes("cool")) return "prep_cool";
    return "prep";
  }
  if (value.includes("heat")) return "heat";
  if (value.includes("cool")) return "cool";
  if (value.includes("fan")) return "fan";
  if (value === "idle" || value === "off") return "idle";
  return value;
}

function pushOperationSample(zoneId: string, sample: OperationSample): void {
  ...
  history.push(sample);
  if (history.length > OPERATION_HISTORY_MAX) history.shift();
  operationHistoryByZone.set(zoneId, history);
}

function recordOperationSample(zoneId: string, system: CarrierSystem, conditioning: string): OperationSample {
...
function recentOperationSamples(zoneId: string): OperationSample[] {
  const cutoff = Date.now() - TRANSITION_WINDOW_MS;
  return (operationHistoryByZone.get(zoneId) ?? []).filter((sample) => sample.at >= cutoff);
}

function hasRecentOperationChange(history: OperationSample[], current: OperationSample): boolean {
  if (history.length < 2) return false;
  const prior = history.slice(0, -1);
  return prior.some((sample) => {
    if (sample.conditioning !== current.conditioning) return true;
    if ((sample.stage === 0) !== (current.stage === 0)) return true;
    if (sample.heatCool !== current.heatCool) return true;
...
function isEnteringConditioning(history: OperationSample[], category: ConditioningCategory): boolean {
  if (!category.startsWith("prep") && category !== "heat" && category !== "cool") return false;
  const prior = history.slice(0, -1);
  return prior.some((sample) => conditioningCategory(sample.conditioning) === "idle");
}

function isLeavingConditioning(history: OperationSample[], category: ConditioningCategory): boolean {
  if (category !== "idle") return false;
  const prior = history.slice(0, -1);
  return prior.some((sample) => {
...
function transitionDirection(history: OperationSample[], current: OperationSample): "up" | "down" | "steady" {
  if (history.length < 2) return "steady";
  const oldest = history[0];
  const currentRpm = current.blwrpm ?? 0;
  const oldestRpm = oldest.blwrpm ?? 0;
  if (current.stage > oldest.stage || currentRpm > oldestRpm + 250) return "up";
  if (current.stage < oldest.stage || (oldestRpm > 350 && currentRpm < oldestRpm - 250)) return "down";
  return "steady";
}

function formatBlowerTelemetry(value: number | null | undefined, unit: "RPM" | "CFM"): string {
  if (value == null || !Number.isFinite(Number(value))) return "Off";
  const rounded = Math.round(Number(value));
  if (rounded <= 0) return "Off";
  return `${rounded} ${unit}`;
}

function blowerTelemetryActive(idu: CarrierSystem["status"]["idu"]): boolean {
  const rpm = idu?.blwrpm ?? 0;
  const cfm = idu?.cfm ?? 0;
  return rpm > 80 || cfm > 50;
}

function buildBlowerMetrics(
...
function formatBlowerDetail(idu: CarrierSystem["status"]["idu"]): string | null {
  const rpm = formatBlowerTelemetry(idu?.blwrpm, "RPM");
  const cfm = formatBlowerTelemetry(idu?.cfm, "CFM");
  return `${rpm} · ${cfm}`;
}

function isBlowerCirculating(idu: CarrierSystem["status"]["idu"]): boolean {
  return blowerTelemetryActive(idu);
}

function presentZoneOperation(
  ...
  let kind: ZoneOperationKind = "idle";
  ...
  if (parsed.defrost) {
    ...
    if (category === "prep_heat") summary = "Getting ready to heat";
    else if (category === "prep_cool") summary = "Getting ready to cool";
    else if (category === "prep") summary = "Getting ready";
    else {
  ...
  if (kind === "heating" || kind === "cooling") {
    if (stage > 0) detailParts.push(`Stage ${stage}`);
    if (blowerDetail) detailParts.push(blowerDetail);
    ...
    const modeLabel =
      fanOnlyMode ? "Fan only" : systemMode === "auto" ? "Auto" : systemMode.charAt(0).toUpperCase() + systemMode.slice(1);
...
function mapActivityZones(system: CarrierSystem): ActivitiesZoneView[] {
  const cfgem = system.status.cfgem;
  const zones: ActivitiesZoneView[] = [];
  ...
  for (const configZone of system.config.zones) {
    ...
    for (const type of ACTIVITY_DISPLAY_ORDER) {
      ...
      activities.push({
        ...
        heat_setpoint_display: formatFahrenheit(activity.htsp, cfgem),
        cool_setpoint_display: formatFahrenheit(activity.clsp, cfgem),
        heat_setpoint: toFahrenheit(activity.htsp, cfgem),
        cool_setpoint: toFahrenheit(activity.clsp, cfgem),
    ...
    for (const activity of configZone.activities) {
      if (ACTIVITY_DISPLAY_ORDER.includes(activity.type as (typeof ACTIVITY_DISPLAY_ORDER)[number])) continue;
      activities.push({
        type: activity.type,
...
function buildActivitiesSnapshot(settings: Settings): ActivitiesSnapshot {
  const system = selectSystem(settings, cachedSystems);
  return {
    connected: Boolean(system && !system.status.isDisconnected),
    configured: isConfigured(settings),
...
function activitiesContent(): string {
  return `
    <div class="toolbar">
      <h2>Activities</h2>
...
async function handleApi(
  ...
  if (route === "/api/settings" && req.method === "PATCH") {
    const body = JSON.parse((await readBody(req)).toString("utf8")) as { system_serial?: string };
    ...
    try {
      ...
      if (
        ...
        const heatSetpoint = formatCarrierSetpoint(
          heat ?? existing?.htsp ?? statusZone?.htsp,
          68,
        );
        const coolSetpoint = formatCarrierSetpoint(
          cool ?? existing?.clsp ?? statusZone?.clsp,
          74,
        );
        ...
        if (fan === "auto" || fan === "on") {
          ...
          fanMode = existing.fan as FanMode;
        }

        await client.updateZoneActivity(serial, zoneId, activityType, heatSetpoint, coolSetpoint, fanMode);
        await refreshCloudData(settings, true);
        const activityZone =
          buildActivitiesSnapshot(settings).zones.find((item) => zoneIdsMatch(item.id, zoneId)) ?? null;
