...
import {
  ...
  discoverRobots,
  exploreIrobotCloudLogin,
  exploreIrobotDiscovery,
  exploreIrobotGigyaLogin,
  exploreIrobotSignedGet,
  exploreRoomba,
  fetchCredentialsFromCloud,
  getDorita980Version,
  getIrobotDiscoveryUrl,
  getLastError,
  getRobotMaintenance,
  getRobotPreferences,
  getRobotStatus,
...
const LUCIDE_PATH = path.join(__dirname, "node_modules/lucide/dist/umd/lucide.min.js");
...
type ApiExplorerSection = "app" | "roomba" | "irobot";

type ApiEndpointDoc = {
  id: string;
  section: ApiExplorerSection;
  method: "GET" | "POST" | "PUT";
  path: string;
  summary: string;
  external?: string;
...
const API_EXPLORER_SECTIONS: Array<{ id: ApiExplorerSection; title: string; hint: string }> = [
  {
    id: "app",
    title: "App",
...
function apiExplorerCardsHtml(): string {
  return API_EXPLORER_SECTIONS.map(
    (section) =>
      `<div class="panel diagnostics-card api-explorer-card">` +
...
const API_ENDPOINTS: ApiEndpointDoc[] = [
  ...
  },
  {
    ...
    external: `GET ${getIrobotDiscoveryUrl()}`,
  ...
  },
  {
    ...
    body: JSON.stringify(
      { fields: ["batPct", "cleanMissionStatus", "bin", "pmaps", "softwareVer", "sku", "lastCommand"] },
      null,
      2,
    ),
...
function layout(page: string, title: string, body: string): string {
  const nav = [
    ...
    .map(
      ([id, label, href]) =>
        `<a href="${href}" class="nav-link ${page === id ? "active" : ""}">${label}</a>`,
    )
    .join("");
...
function apiExplorerPage(): string {
...
const ROOMBA_EXPLORE_OPERATIONS = new Set<RoombaExploreOperation>([
  "get-state",
  "preferences",
  "wireless-status",
  "cloud-config",
...
async function handleExploreRequest(
  req: IncomingMessage,
  res: ServerResponse,
  pathname: string,
  method: string,
  ...
  try {
    ...
    if (method === "GET" && pathname === "/api/explore/irobot/discovery") {
      result = await exploreIrobotDiscovery();
    } else if (method === "POST" && pathname === "/api/explore/irobot/gigya-login") {
      const body = await readJson<{ username?: string; password?: string }>(req);
      result = await exploreIrobotGigyaLogin(settings, body.username, body.password);
    } else if (method === "POST" && pathname === "/api/explore/irobot/cloud-login") {
      const body = await readJson<{ username?: string; password?: string }>(req);
      result = await exploreIrobotCloudLogin(settings, body.username, body.password);
    } else if (method === "GET" && pathname === "/api/explore/irobot/pmaps") {
      const url = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`);
      result = await exploreIrobotSignedGet(settings, `/v1/${settings.blid.trim()}/pmaps`, url.searchParams.toString());
    } else if (method === "GET" && pathname === "/api/explore/irobot/smartcleanfavorites") {
      result = await exploreIrobotSignedGet(
        settings,
        `/v1/${settings.blid.trim()}/smartcleanfavorites`,
      );
    } else if (method === "GET" && pathname === "/api/explore/irobot/favorites") {
      result = await exploreIrobotSignedGet(settings, `/v1/${settings.blid.trim()}/favorites`);
    } else if (method === "POST" && pathname.startsWith("/api/explore/roomba/")) {
      const operation = pathname.replace("/api/explore/roomba/", "") as RoombaExploreOperation;
      if (!ROOMBA_EXPLORE_OPERATIONS.has(operation)) {
        sendJson(res, 404, { error: "Unknown Roomba explore operation" });
        return true;
      }
      const body = await readJson<{ fields?: string[]; command?: Record<string, unknown> }>(req);
      result = await exploreRoomba(settings, operation, body);
    } else {
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && pathname === "/lucide.min.js") {
    try {
      const script = await readFile(LUCIDE_PATH);
      res.writeHead(200, {
  ...
  if (method === "POST" && pathname === "/api/setup/discover") {
    try {
      ...
      try {
        const body = await readJson<{ robot_ip?: string }>(req);
        if (body.robot_ip?.trim()) robotIpHint = body.robot_ip.trim();
      } catch {
        // ignore empty or invalid discover request bodies
      }
      const robots = await discoverRobots(robotIpHint);
