...
export type ParseResult = {
  format: "hourly_csv" | "tou_csv" | "history_csv";
  utility: Utility;
  filename: string;
  account_id: string | null;
  usage_point: string | null;
...
function resolveHourColumns(header: string[]): { hourStart: number; hourCount: number } {
  let hourStart = 1;
...
function parseTouCsv(content: string, filename: string): ParseResult {
  const { account_id, usage_point } = filenameIds(filename);
  const utility = filenameUtility(filename) ?? "electric";
  const unit: "kWh" | "gal" = utility === "water" ? "gal" : "kWh";
  const lines = content.trim().split(/\r?\n/).filter(Boolean);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!tierName || /^\d+$/.test(tierName)) continue;
...
function isTouCsv(filename: string, content: string): boolean {
  const lower = filename.toLowerCase();
  ...
  if (header.includes("tier")) return true;
...
export function detectFormat(filename: string, content: string): "hourly_csv" | "tou_csv" | "history_csv" {
...
export function parseNbuExport(filename: string, content: string): ParseResult {
  ...
  if (format === "tou_csv") return parseTouCsv(content, filename);
