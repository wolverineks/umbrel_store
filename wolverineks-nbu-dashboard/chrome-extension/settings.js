const NBU_PROFILES = ["prod", "dev"];

const NBU_PROFILE_LABELS = {
  prod: "Production",
  dev: "Development",
};
...
async function getActiveUmbrelSettings() {
  ...
  if (!baseUrl || !token) {
    const label = NBU_PROFILE_LABELS[activeProfile];
    throw new Error(
      `Configure Umbrel URL and ingest token for ${label} in the extension popup.`,
    );
  }
  return { baseUrl, token, activeProfile, profileLabel: NBU_PROFILE_LABELS[activeProfile] };
}